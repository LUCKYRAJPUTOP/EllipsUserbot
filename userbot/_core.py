...
dark_logo = "./Harsh-78/Darkbot_logo.jpg"
h1m4n5hu0p = darkbot.uid
dark = f"[{DEFAULTUSER}](tg://user?id={h1m4n5hu0p})"

@darkbot.on(admin_cmd(pattern=r"send (?P<shortname>\w+)", outgoing=True))
@darkbot.on(sudo_cmd(pattern=r"send (?P<shortname>\w+)", allow_sudo=True))
async def send(event):
    ...
    omk = f"** 𖣘︎Plugin name ≈** `{input_str}`\n**❥ Uploaded by ≈** {dark}\n\n⚡ **[𝙼𝙰𝚂𝚃𝙴𝚁 𝙰𝚃 𝙳𝙰𝚁𝙺𝙱𝙾𝚃](t.me/Dark_Bot_updates)** 🔥"
...
@darkbot.on(admin_cmd(pattern="install$", outgoing=True))
@darkbot.on(sudo_cmd(pattern="install$", allow_sudo=True))
                ...
                if shortname in CMD_LIST:
                    ...
                    for i in CMD_LIST[shortname]:
                        ...
                        if b == 1:
                            ...
                            b = 1
                        await event.edit(a)
                    return await event.edit(f"✅ **Installed module** :- `{shortname}` \n✨ BY :- {dark}\n\n{string}\n\n        ⚡ **[𝙼𝙰𝚂𝚃𝙴𝚁 𝙰𝚃 𝙳𝙰𝚁𝙺𝙱𝙾𝚃](t.me/Dark_bot_updates)** 🔥", link_preview=False)
...
@darkbot.on(admin_cmd(pattern=r"uninstall (?P<shortname>\w+)", outgoing=True))
@darkbot.on(sudo_cmd(pattern=r"uninstall (?P<shortname>\w+)", allow_sudo=True))
...
@darkbot.on(admin_cmd(pattern=r"unload (?P<shortname>\w+)$"))
@darkbot.on(sudo_cmd(pattern=r"upload (?P<shortname>\w+)$", allow_sudo=True))
...
@darkbot.on(admin_cmd(pattern=r"load (?P<shortname>\w+)$"))
@darkbot.on(sudo_cmd(pattern=r"load (?P<shortname>\w+)$", allow_sudo=True))
...
CmdHelp("core").add_command(
  ...
  "cmds", None, "Gives out the list of modules in darkbot."
).add()
