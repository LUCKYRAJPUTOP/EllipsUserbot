...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            ellipsresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            ellipsfile = await event.client.download_media(ellipsresponse, "./temp")
            return await unzip(ellipsfile)
