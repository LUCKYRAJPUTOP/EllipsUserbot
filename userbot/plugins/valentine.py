...
from darkbot import CmdHelp, bot as darkbot, ALIVE_NAME
from darkbot.utils import admin_cmd, sudo_cmd, edit_or_reply as eor

DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "Your Lover"

h1m4n5hu0p = darkbot.uid

dark = f"[{DEFAULTUSER}](tg://user?id={h1m4n5hu0p})"

@darkbot.on(admin_cmd(pattern="hvd$"))
@darkbot.on(sudo_cmd(pattern="hvd$", allow_sudo=True))
...
@darkbot.on(admin_cmd(pattern="vday$"))
@darkbot.on(sudo_cmd(pattern="vday$", allow_sudo=True))
async def gif(event):
    if event.fwd_from:
        return
