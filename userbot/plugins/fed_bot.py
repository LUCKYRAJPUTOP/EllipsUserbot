...
from darkbot.plugins.sql_helper.fban_sql import (
    add_channel,
    get_all_channels,
    in_channels,
    rm_channel,
...
@darkbot.on(admin_cmd(pattern="fban ?(.*)"))
@darkbot.on(sudo_cmd(pattern="fban ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@darkbot.on(admin_cmd(pattern="unfban ?(.*)"))
@darkbot.on(sudo_cmd(pattern="unfban ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mssg = await eor(event, "`...`")
...
@darkbot.on(admin_cmd(pattern=r"fadd ?(.*)"))
@darkbot.on(sudo_cmd(pattern=r"fadd ?(.*)", allow_sudo=True))
...
@darkbot.on(admin_cmd(pattern=r"fremove ?(.*)"))
@darkbot.on(sudo_cmd(pattern=r"fremove ?(.*)", allow_sudo=True))
...
@darkbot.on(admin_cmd(pattern="fgroups"))
@darkbot.on(sudo_cmd(pattern="fgroups", allow_sudo=True))
...
@darkbot.on(admin_cmd(pattern="fsearch ?(.*)"))
@darkbot.on(sudo_cmd(pattern="fsearch ?(.*)", allow_sudo=True))
...
@darkbot.on(admin_cmd(pattern="newfed ?(.*)", outgoing=True))
@darkbot.on(sudo_cmd(pattern="newfed ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await event.client.send_message(chat, f"/newfed {dark_input}")
...
@darkbot.on(admin_cmd(pattern="renamefed ?(.*)"))
@darkbot.on(sudo_cmd(pattern="renamefed ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return 
    dark_input = event.pattern_match.group(1)
    ...
    async with event.client.conversation(chat) as conv:
          try:     
              ...
              await event.client.send_message(chat, f"/renamefed {dark_input}")
              response = await response 
          except YouBlockedUserError: 
...
@darkbot.on(admin_cmd(pattern="fstat ?(.*)"))
@darkbot.on(sudo_cmd(pattern="fstat ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    dark = await event.edit("`Collecting fstat....`")
    thumb = dark_logo
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
        ...
        async with borg.conversation(bot) as conv:
            try:
                ...
                if "Looks like" in massive.text:
                    ...
                    massive = await conv.get_response()
                    await darkbot.send_file(
                        event.chat_id,
                        massive,
                        thumb=thumb,
                        caption=f"List of feds {user} has been banned in.\n\n**⚡ [Collected using darkbot](t.me/darkbot_Support) ⚡**",
                    )
                else:
                    await borg.send_message(event.chat_id, massive.text)
                await event.delete()
            except YouBlockedUserError:
                await dark.edit("`Please Unblock` @MissRose_Bot")


@darkbot.on(admin_cmd(pattern="fedinfo ?(.*)"))
@darkbot.on(sudo_cmd(pattern="fedinfo ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    dark = await eor(event, "`Fetching fed info.... please wait`")
    lavde = event.pattern_match.group(1)
    async with borg.conversation(bot) as conv:
        try:
            await conv.send_message("/start")
            ...
            massive = await conv.get_response()
            await dark.edit(massive.text + "\n\n**LEGENDARY_AF_darkbot**")
        except YouBlockedUserError:
            await dark.edit("`Please Unblock` @MissRose_Bot")
